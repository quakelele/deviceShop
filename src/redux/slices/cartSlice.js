...
export const addToCart = createAsyncThunk("cart/addToCart", async (obj, { dispatch, getState }) => {
...
export const plusQuantity = createAsyncThunk('resource/put', async (obj, { dispatch }) => {
    if (obj.quantity >= 5) {
        return obj
    }
    const response = await fetch(
        `http://localhost:3005/cart/${obj.id}`
        , {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
...
export const minusQuantity = createAsyncThunk('resource/put', async (obj, { dispatch }) => {
    if (obj.quantity <= 1) {
        // return dispatch(deleteFromCartBack(obj.id))
        return obj
    }
    const response = await fetch(
        `http://localhost:3005/cart/${obj.id}`
...
export const deleteFromCartBack = createAsyncThunk("cart/deleteFromCartBack", async (id, { dispatch }) => {
